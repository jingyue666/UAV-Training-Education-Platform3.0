...
def show_basic_info():
    try:
        ...
        c = conn.cursor()
        c.execute("SELECT username, real_name, phone, role, level FROM users WHERE id=?",
...
# ================== 记录观看历史（给 training 调用） ==================
def add_watch_history(title, module_name):
    if "watch_history" not in st.session_state:
        st.session_state.watch_history = []

    # 避免重复记录
    exist = next((x for x in st.session_state.watch_history if x['title'] == title), None)
    if not exist:
        ...
        if len(st.session_state.watch_history) > 10:
            st.session_state.watch_history.pop(0)import streamlit as st
import sqlite3
import os
