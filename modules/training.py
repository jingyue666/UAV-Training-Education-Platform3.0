...
DEFAULT_COVERS = {
    ...
    "行业应用模块": "https://images.unsplash.com/photo-1531259683007-009259683007?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80",
...
def show():
    ...
    # 兜底初始化，杜绝None
    if "mod" not in st.session_state or not st.session_state.mod:
    ...
    try:
        ...
        for i, (title, cover, url, level) in enumerate(videos):
            with cols[i % 3]:
                ...
                if st.button("播放", key=f"play_btn_{i}", use_container_width=True):
